...
module.exports = async (req, res) => {
  ...
  try {
    const requestedPublishId = req.query?.publish_id || req.body?.publish_id;
    if (requestedPublishId && (req.method === "GET" || req.body?.action === "status")) {
      const result = await getPublishStatus(session.access_token, String(requestedPublishId));
      ...
      return res.status(200).json({ success: true, mode: session.mode, publish_id: String(requestedPublishId), status: d.status, fail_reason: d.fail_reason || null, publicly_available_post_id: d.publicaly_available_post_id || [], data: d });
    }
...
<div id="postForm"><label>Video từ máy</label><input id="video" type="file" accept="video/mp4,video/quicktime,video/webm"><div class="note">Dùng FILE_UPLOAD nên không cần xác minh domain của video.</div>
...
<button id="submit" type="button">Đăng lên TikTok</button></div><div id="status" class="status" hidden></div>
<script src="/tiktok-post.js?v=5" defer></script></body></html>`);
    }
    if (req.method === "POST") {
      ...
      const initData=await initResponse.json(); console.log('TikTok Direct Post FILE_UPLOAD Init:',initData);
      if(!initResponse.ok||initData.error?.code!=='ok'){const e=initData.error||{};return res.status(initResponse.status||400).json({success:false,message:'TikTok Direct Post initialization failed',http_status:initResponse.status,code:e.code,error_message:e.message,log_id:e.log_id||e.logid,error:initData});}
