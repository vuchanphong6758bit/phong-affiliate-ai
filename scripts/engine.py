...
Trả JSON: title, body, hypothesis, cta. Body 500-900 từ tiếng Việt, CTA rõ, URL đúng 1 lần.''';r=client.responses.create(model=os.getenv("OPENAI_MODEL","gpt-5.6"),input=prompt)
 ...
 except:return {"title":product["name"],"body":r.output_text,"hypothesis":"Tối ưu góc lợi ích và CTA","cta":"Xem sản phẩm"}
def publish(payload):
...
def main():
 ...
 p=products[0];now=datetime.now(VN).isoformat();post=generate(p,context(c));published=publish({**post,"product":p});c.execute("INSERT OR REPLACE INTO products VALUES(?,?,?,?,?,?,?,?,?)",(p["id"],p["name"],p["url"],p["price"],p["commission_rate"],p["commission"],p["score"],json.dumps(p["metadata"],ensure_ascii=False),now));c.execute("INSERT INTO posts(product_id,title,body,published_url,created_at) VALUES(?,?,?,?,?)",(p["id"],post.get("title",""),post.get("body",""),published,now));c.commit();state=json.loads(STATE.read_text(encoding="utf-8")) if STATE.exists() else {"posts":[],"metrics":[]};state["posts"].append({"product_id":p["id"],"title":post.get("title",""),"published_url":published,"created_at":now});state["posts"]=state["posts"][-90:];STATE.write_text(json.dumps(state,ensure_ascii=False,indent=2),encoding="utf-8");c.close();print(json.dumps({"product":p,"post":post,"published_url":published},ensure_ascii=False))
